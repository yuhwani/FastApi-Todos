...
def normalize_todo(todo: dict) -> dict:
    todo.setdefault("comments", [])
    todo.setdefault("start_date", None)
    todo.setdefault("end_date", None)
    return todo
...
def create_todo(todo: TodoItem) -> dict:
    todos = load_todos()
...
def update_todo(todo_id: int, updated_todo: TodoItem) -> dict:
    todos = load_todos()
...
def add_comment(todo_id: int, comment: CommentCreate) -> dict:
    todos = load_todos()
    ...
    for todo in todos:
        if todo["id"] == todo_id:
            ...
            comments.append({"id": new_comment_id, "content": comment.content})
